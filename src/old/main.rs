...
pub fn main() {
    let _login_file = login::LoginInfo::serialize_from_file();
    App::new()
        .insert_resource(NetworkSettings {
            connection_mode: ConnectionMode::Offline,
            ..Default::default()
        })
        .add_command::<login::LoginCommand>()
        .add_command::<admin::GamemodeCommand>()
        .add_command::<admin::TeleportCommand>()
        ...
        .add_systems(
            Update,
            (
                login::handle_test_command,
                admin::handle_gamemode_command,
                admin::handle_teleport_command,
                init_clients,
...
fn setup(
    ...
    world::worldgen(&mut layer, &mut commands);
    world::build_spawn_box(&mut layer, SPAWN_BOX, &mut commands);
...
fn init_clients(
    ...
    ) in &mut clients
    {
        ...
        *game_mode = GameMode::Adventure;
        ...
        unsafe {
            client.send_chat_message(
                "You are ".into_text() +  format!("{PLAYER_COUNT}").into_text().bold().color(Color::GREEN) + " player on this server".into_text(),
            )
        }
    }
}
